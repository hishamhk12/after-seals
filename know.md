...
44. تفاصيل تشغيلية إضافية لخدمة التوصيل – من دليل المستخدم "آلية عمل خدمة التوصيل في Odoo"

بالإضافة لكل اللي فوق، في عندنا دليل مستخدم رسمي (PDF) بعنوان "آلية عمل خدمة التوصيل في Odoo" (إعداد Feras Zitunah) بيوثق تفاصيل تشغيلية أعمق لنفس خدمة التوصيل. هاي التفاصيل مكمّلة/مساندة للمسار الحالي المرئي المكوّن من سبع مراحل (فاتورة من SAP → طلب توصيل → جدولة التوصيل → ربط الخدمة بالسائق → ملئ النموذج → جاري التوصيل → استلام الخدمة)، وما بتستبدله ولا بتغيّره.

أهم النقاط الموثقة في الدليل:

- الفاتورة يتم إنشاؤها وفوترتها في SAP، وبعدين تنتقل/تتكامل إلى Odoo لتبدأ عليها إجراءات خدمة التوصيل.
- أول ما توصل الفاتورة لـOdoo بتنحط بمرحلة "طلب توصيل"، وبعد فترة زمنية محددة بتنتقل تلقائيًا لـ"جدولة التوصيل"؛ وإذا ما انتقلت تلقائيًا فممكن يكون السبب ارتباطها بخدمة ثانية أو وجود إجراء إضافي مرتبط فيها.
- بمرحلة جدولة التوصيل، النظام بيرسل تلقائيًا رسالة WhatsApp للعميل فيها رابط أو آلية حجز موعد التوصيل، والعميل بيختار الموعد المناسب إله.
- في الحجز، في مفهوم Daily Capacity (الطاقة الاستيعابية اليومية) بينحدد لكل صالة أو موقع، مثلاً 10 فواتير توصيل باليوم. لما عدد الحجوزات ليوم معين يوصل للسقف المحدد، النظام تلقائيًا بيقفل هذا التاريخ للحجز، ما بيسمح للعميل يختاره، وبيلزمه يختار موعد ثاني متاح. هيك بيضمنوا توزيع عمليات التوصيل حسب الطاقة الاستيعابية المحددة لكل يوم وموقع.
- بعد ما العميل يحجز موعده، الفاتورة بتنتقل لمرحلة "ربط الخدمة بالسائق": المسؤول عن التوصيل بيفتح الفاتورة، يراجع تفاصيل حجز العميل، يختار السائق المسؤول، يربطه بالفاتورة، وبيحفظ البيانات.
- بعد ربط السائق، بتظهر تفاصيل الحجز كاملة: تاريخ التوصيل، وقت التوصيل، بيانات العميل، موقع العميل، تفاصيل الفاتورة، وبيانات السائق.
- بعدها بتنتقل الفاتورة لمرحلة تجهيز وتسليم الشحنة للسائق: المسؤول بيسلّم الشحنة للسائق، والسائق بيستلم الفاتورة/الشحنة، يحمّلها عالسيارة، يتأكد من بيانات التوصيل، وبيبدأ فعليًا بتنفيذ عملية التوصيل. من هاي النقطة بتبدأ مهمة السائق الفعلية.
- السائق بيدخل نظام Odoo (بوابة سائق مخصصة) باستخدام Username وPassword، وبعد ما يتأكد من بيانات المهمة بيضغط "Start / بدء الرحلة"، وعندها النظام بيرسل تلقائيًا إشعار WhatsApp للعميل إنه الشحنة أصبحت بطريقها إله.
- لما السائق يوصل لموقع العميل: بيسلّم الشحنة له، بيرفع صورة إثبات تسليم من نفس الموقع، يتأكد إن عملية التسليم تمت، وبيضغط "End Task / نهاية الرحلة".
- بعد End Task، النظام بيرسل تلقائيًا رمز OTP للعميل لتأكيد الاستلام: العميل بيعطي الرمز للسائق، السائق بيدخله بالنظام، النظام يتحقق من صحته، وبعد نجاح التحقق بيتأكد استلام الشحنة، خدمة التوصيل تعتبر مكتملة، وبتنقفل رحلة التوصيل.

مهم: كل هالتفاصيل مأخوذة من الدليل الرسمي وبتنعتبر confirmed/documented لأنها موثقة بمصدر رسمي، بس هي تفاصيل تشغيلية إضافية جوا وبين المراحل السبع الأساسية، ومش المفروض تُستخدم لتغيير أو استبدال المسار السبع مراحل المعروض حاليًا بالواجهة. أي سؤال عام عن "شو فلو التوصيل" لازم يُجاوب بالمسار السبع مراحل الحالي، وأي سؤال تفصيلي (متل Daily Capacity أو شو بيصير بعد ربط السائق أو شو بيعمل السائق بعد استلام الشحنة أو شو بصير بعد End Task) بيستفيد من هاي التفاصيل الموثقة بالدليل.
